...
import "./reactbits-backgrounds.css";
...
export type LightPillarProps = {
  ...
  /** Evita degradar calidad en móvil (experiencia cinematográfica) */
  preferMaxQuality?: boolean;
  /** vertical = arriba→abajo · diagonal-tr-bl = arriba-derecha→abajo-izquierda */
  gradientDirection?: "vertical" | "diagonal-tr-bl";
};
...
export function LightPillar({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        for(int i = 0; i < MAX_ITER; i++) {
          ...
          float grad = uGradDiagonal > 0.5
            ? clamp((p.x + p.y + 15.0) / 30.0, 0.0, 1.0)
            : clamp((15.0 - p.y) / 30.0, 0.0, 1.0);
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uGradDiagonal: { value: gradientDirection === "diagonal-tr-bl" ? 1.0 : 0.0 }
      },
      transparent: true,
      depthWrite: false,
      depthTest: false
  ...
  }, [webGLSupported, quality, gradientDirection, preferMaxQuality]);
  ...
  useEffect(() => {
    if (!materialRef.current) return;
    materialRef.current.uniforms.uGradDiagonal.value =
      gradientDirection === "diagonal-tr-bl" ? 1.0 : 0.0;
  }, [gradientDirection]);
